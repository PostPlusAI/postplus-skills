...
```bash
node skills/image-batch-runner/scripts/upload_media.mjs --request path/to/upload-request.json
```

Submit video with:

```bash
node skills/video-batch-runner/scripts/generate_video_from_image_audio.mjs --request path/to/request.seed.json
```

Poll with:

```bash
node skills/video-batch-runner/scripts/poll_prediction.mjs --request path/to/request.json
```
