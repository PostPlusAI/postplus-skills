...
function routeBrief(brief) {
  ...
  if (brief.inputType === 'audio') {
    ...
    route.why.push(
      'Transcript already exists, so packaging should happen before any re-transcription.',
    );
  } else if (brief.inputType === 'transcript+assets') {
    route.primarySkill = 'editing-decision-engine';
    route.why.push(
      'A transcript plus media assets means the main problem is edit planning, not transcription.',
    );
  }
  ...
  if (brief.goal === 'subtitles') {
    ...
    brief.goal === 'beats' ||
    brief.goal === 'broll-map' ||
    brief.goal === 'edit-plan'
  ) {
    route.route = 'edit-prep';
    if (
      !route.supportingSkills.includes('video-analysis') &&
      brief.inputType === 'video'
    ) {
      route.supportingSkills.push('video-analysis');
    }
    ...
    route.why.push(
      'Edit-prep outputs need transcript timing plus downstream decision logic.',
    );
  } else {
    route.route = 'transcript-only';
    route.why.push(
      'The requested output does not require extra semantic or edit-planning stages.',
    );
  }

  if (
    needsTimestamps &&
    !route.outputArtifacts.includes('timed-transcript.json')
  ) {
  ...
  if (brief.scale === 'batch') {
    route.why.push(
      'Batch work should persist per-asset manifests and support retries.',
    );
  }
