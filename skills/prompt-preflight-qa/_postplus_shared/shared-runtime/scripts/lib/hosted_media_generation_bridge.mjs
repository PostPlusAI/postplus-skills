...
import { runHostedCapabilityRequest } from "./hosted_capability_bridge.mjs";
...
export async function requestHostedMediaGenerationJson(
  ...
  if (!requestDimensions || typeof requestDimensions !== "object") {
    throw new Error("requestDimensions is required for hosted media generation.");
  }

  const data = await runHostedCapabilityRequest({
