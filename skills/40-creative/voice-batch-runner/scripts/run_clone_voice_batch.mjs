...
const DEFAULT_INITIAL_POLL_DELAY_MS = 0;
const DEFAULT_POLL_INTERVAL_MS = 5000;
const DEFAULT_MAX_POLL_ROUNDS = 0;

function usage() {
  console.error(
    'Usage: node run_clone_voice_batch.mjs --batch <batch-manifest.json> [--submit-interval-ms 1000] [--initial-poll-delay-ms 0] [--poll-interval-ms 5000] [--max-poll-rounds 0]',
  );
}
...
function buildManifest(request, paths, responsePayload, extras = {}) {
  ...
  return {
    ...
    referenceAudioUrl:
      extras.referenceAudioUrl || request.referenceAudioUrl || null,
...
async function submitOne(requestPath, uploadCache) {
  ...
  return {
    ...
    pending:
      ...
      manifest.providerStatus !== 'failed',
  };
}
...
async function pollOne(item) {
  ...
  if (!resultUrl) {
    const manifest = buildManifest(request, paths, priorResponse, {
      ...
      referenceAudioUrl:
        item.manifest?.referenceAudioUrl || request.referenceAudioUrl || null,
...
async function main() {
  ...
  const submitIntervalMs = toInt(
    args['submit-interval-ms'],
    DEFAULT_SUBMIT_INTERVAL_MS,
  );
  const initialPollDelayMs = toInt(
    args['initial-poll-delay-ms'],
  ...
  for (let index = 0; index < items.length; index += 1) {
    ...
    console.log(
      `[submit ${index + 1}/${items.length}] ${submittedItem.manifest.jobId} -> ${submittedItem.manifest.providerStatus}`,
    );
  ...
  for (let round = 0; round < maxPollRounds && pending.length > 0; round += 1) {
    ...
    for (const item of pending) {
      ...
      console.log(
        `[poll ${round + 1}] ${polled.manifest?.jobId || item.requestPath} -> ${status}`,
      );
      if (polled.pending) {
  ...
  const summary = {
    ...
    completed: finalItems.filter((item) => item.providerStatus === 'completed')
      .length,
    failed: finalItems.filter((item) => item.providerStatus === 'failed')
      .length,
    pending: finalItems.filter(
      (item) =>
        item.providerStatus !== 'completed' && item.providerStatus !== 'failed',
    ).length,
  };
