...
function classifyAttentionOwner(beat, candidate) {
  ...
  if (
    ...
    ['workflow-bridge', 'transition-cover'].includes(beat.visualNeed)
  ) {
...
function inferARollAction(attentionOwner, beat) {
  ...
  if (
    attentionOwner === 'transition-motion' &&
    beat.coverageStyle !== 'stay-on-face'
  ) {
...
function inferBrollAction(attentionOwner, beat, candidate) {
  if (
    !candidate ||
    attentionOwner === 'a-roll-face' ||
    attentionOwner === 'subtitle-keyword'
  ) {
    return {
      mode: 'none',
      assetId: null,
  ...
  const confidence = isStrongCandidate(candidate)
    ? 'high'
    : isMediumCandidate(candidate)
      ? 'medium'
      : 'low';
...
function inferKeywordEmphasis(beat, attentionOwner) {
  const keywords = Array.isArray(beat.keywordOverlay)
    ? beat.keywordOverlay
    : [];
...
function inferMicroMotion(beat, attentionOwner) {
  ...
  const preset =
    beat.motionHint && beat.motionHint !== 'none'
      ? beat.motionHint
      : 'hold-clean';
...
function buildPackage({
  ...
  const beats = Array.isArray(brollPlan.beats)
    ? brollPlan.beats.map(enhanceBeat)
    : [];

  return {
    schemaVersion: 'edit-enhancement-package/v1',
    packageId:
      slugify(`${brollPlan.planId || 'broll-plan'}-enhancement`) ||
      'edit-enhancement-package',
    ...
    meta: {
      createdAt: nowIso(),
      generator:
        'skills/40-creative/b-roll-family/edit-enhancement-packager/scripts/run_package_edit_enhancements.mjs',
      outputPath: path.resolve(outputPath),
      beatCount: beats.length,
      attentionOwnerCounts: beats.reduce((accumulator, beat) => {
        accumulator[beat.attentionOwner] =
          (accumulator[beat.attentionOwner] || 0) + 1;
        return accumulator;
      }, {}),
    },
  };
...
async function main() {
  ...
  const outputPath = path.resolve(
    args.output ||
      path.join(path.dirname(brollPlanPath), 'edit-enhancement-package.json'),
  );
