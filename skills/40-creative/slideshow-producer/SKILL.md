...
## Fail Fast
- No vibe, fewer than two slides, missing local image/reference path, invalid
  manifest routing, no source basis/output path, missing Pillow, missing hosted
  image capability, or user has not approved generation.
- Do not auto-generate, call providers directly, or hide routing problems with a
  fallback text-to-image pass.

## Public Command Boundary

- Check readiness first: `postplus doctor --skill slideshow-producer`.
