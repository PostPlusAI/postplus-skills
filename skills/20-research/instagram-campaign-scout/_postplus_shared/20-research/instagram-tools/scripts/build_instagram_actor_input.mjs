...
function hashtagToUrl(hashtag) {
  ...
  return normalized
    ? `https://www.instagram.com/explore/tags/${normalized}/`
    : null;
}
...
function buildInput(brief, sourceId) {
  ...
  const queries = normalizeStringArray(
    brief.queries || brief.searchQueries || brief.keywords,
  );
  const hashtags = normalizeStringArray(brief.hashtags);
  const usernames = normalizeStringArray(
    brief.usernames || brief.profiles || brief.handles,
  );
  const urls = normalizeStringArray(
    brief.urls || brief.postUrls || brief.directUrls,
  );
  const limit = Number(
    brief.limit ||
      brief.maxItems ||
  ...
  if (
    ...
    actor.includes('instagram-search-scraper')
  ) {
  ...
  if (
    ...
    actor.includes('instagram-profile-scraper')
  ) {
    return {
      ...extra,
      usernames,
  ...
  if (actor === 'instagram-posts' || actor.includes('instagram-post-scraper')) {
  ...
  if (
    actor === 'instagram-hashtags' ||
    actor.includes('instagram-hashtag-scraper')
  ) {
    return {
      ...extra,
      hashtags,
  ...
  if (actor.includes('instagram-tagged-scraper')) {
    const taggedUsernames = targetUsername ? [targetUsername] : usernames;
    return {
      ...extra,
  ...
  if (
    actor === 'instagram-comments' ||
    actor.includes('instagram-comment-scraper')
  ) {
    return {
      ...extra,
      directUrls: urls,
  ...
  if (
    actor === 'instagram-email-search' ||
    actor.includes('instagram-email-scraper')
  ) {
    const keyword = queries[0] || hashtags[0] || '';
    const limitText = Number.isFinite(limit) && limit > 0 ? String(limit) : '';

    if (!keyword) {
      throw new Error(
        'instagram-email-scraper requires at least one query, searchQuery, keyword, or hashtag.',
      );
    }
    ...
    return {
      ...
      Email_Type: '0',
      Country: cleanString(brief.country) || 'www',
      Limit: limitText,
    };
  }
  ...
  if (actor.includes('instagram-scraper')) {
    return {
      ...
      directUrls: [
        ...urls,
        ...usernames
          .map((username) => usernameToProfileUrl(username))
          .filter(Boolean),
        ...hashtags.map((hashtag) => hashtagToUrl(hashtag)).filter(Boolean),
      ],
      resultsLimit: limit,
