...
export const DEFAULT_SUPPORTED_PLATFORMS = Object.freeze([
  'linkedin',
  'youtube',
  'facebook',
]);

export function normalizeSupportedPlatforms(
  platforms = DEFAULT_SUPPORTED_PLATFORMS,
) {
  const normalized = Array.from(
    new Set(
      (Array.isArray(platforms) ? platforms : []).map((item) =>
        String(item || '').trim(),
      ),
    ),
  ).filter(Boolean);
  ...
  if (normalized.length === 0) {
    throw new Error(
      'supportedPlatforms must contain at least one Public content platform',
    );
  }

  const invalid = normalized.filter(
    (item) => !DEFAULT_SUPPORTED_PLATFORMS.includes(item),
  );
  if (invalid.length > 0) {
...
export function formatSupportedPlatforms(
  platforms = DEFAULT_SUPPORTED_PLATFORMS,
) {
...
export function extractUrls(text) {
  return Array.from(
    new Set(
      (String(text || '').match(/https?:\/\/[^\s]+/g) || []).map((item) =>
        item.trim(),
      ),
    ),
  );
...
const BILINGUAL_COUNT_UNITS = Object.freeze([
  'result',
  'results',
  'post',
  'posts',
...
export function detectPlatforms(text) {
  ...
  const platforms = [];
  for (const [platform, terms] of Object.entries(
    BILINGUAL_SUPPORTED_PLATFORM_TERMS,
  )) {
...
export function detectUnsupportedPlatforms(text) {
  ...
  const unsupported = [];
  for (const [platform, terms] of Object.entries(
    BILINGUAL_UNSUPPORTED_PLATFORM_TERMS,
  )) {
    if (includesBilingualTerm(value, terms)) {
...
export function parseRequestedCount(text, fallback = 50) {
  ...
  const directMatch = value.match(
    new RegExp(`(\\d+)\\s*(${countUnitPattern})`, 'i'),
  );
  if (directMatch) {
  ...
  const looseMatch = value.match(
    new RegExp(`(\\d+)[^\\d]{0,20}(${countUnitPattern})`, 'i'),
  );
  if (looseMatch) {
...
export function isPublicContentPendingResult(value) {
  return Boolean(
    value &&
      typeof value === 'object' &&
      !Array.isArray(value) &&
...
export async function publicContentSnapshotStatus({ snapshotId }) {
...
export function summarizePlatformItems(items) {
  ...
  return {
    ...
    withLikes: normalizedItems.filter((item) => item.metrics.likes !== null)
      .length,
    withViews: normalizedItems.filter((item) => item.metrics.views !== null)
      .length,
  };
}

export async function discoverPublicContent({
  discoveryTool,
  args,
  fetchImpl = globalThis.fetch,
}) {
